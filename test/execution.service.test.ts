...
describe('execution.service', () => {
  ...
  it('wraps identityBlock/negativePrompt with the character\'s Prompt Adapter before phase-prompt wrapping', async () => {
    const charactersDir = path.join(dir, 'characters');
    const characters = createCharactersService(charactersDir);
    const character = characters.create({
      name: 'Rin Takahashi',
      checkpoint: 'illustriousXL_v1',
      identityBlock: 'photo of a woman',
    ...
    workflowMapping.updateNodeMapping('003-Cleanup', version, '2', 'text', {
      sourceType: 'domain',
      sourceValue: 'character.identityBlock',
      status: 'mapped',
    });
    workflowMapping.updateNodeMapping('003-Cleanup', version, '3', 'text', {
      sourceType: 'domain',
      sourceValue: 'character.negativePrompt',
      status: 'mapped',
    });
    workflowMapping.bindPhase('003-Cleanup', version, '003-Cleanup');
    workflowMapping.setResultOutput('003-Cleanup', version, {
    ...
    try {
      ...
      const executionService = createExecutionService({
        ...
        clientId: 'app-client',
        phasePromptConfig: {
          refinement_cleanup: { prefix: '', suffix: ', plain white background' },
        },
      });

      await executionService.submitSingle(character.slug, 'refinement_cleanup');

      // Adapter segments (lead/quality tags) sit innermost, ahead of the identity block;
      // the phase-prompt suffix wraps around the outside of that, unchanged from the
      // existing phase-prompt-only behavior. The negative prompt gets its own adapter
      // wrap, with no phase-prompt involvement.
      expect(submittedGraph?.['2'].inputs?.text).to.equal(
        '1girl, masterpiece, best quality, photo of a woman, plain white background',
      );
      expect(submittedGraph?.['3'].inputs?.text).to.equal('worst quality, blurry, watermark');
    } finally {
      socket.close();
      await new Promise<void>((resolve) => wsStub.wss.close(() => resolve()));
      await httpStub.close();
  ...
  it('resolves the negative prompt to an empty string when the Prompt Adapter suppresses negatives', async () => {
    const charactersDir = path.join(dir, 'characters');
    const characters = createCharactersService(charactersDir);
    const character = characters.create({
      name: 'Rin Takahashi',
      checkpoint: 'flux1-dev',
      identityBlock: 'a woman standing in a sunlit garden',
      negativePrompt: 'blurry, watermark',
      promptAdapter: {
        presetId: 'flux',
        leadTags: '',
    ...
    try {
      ...
      expect(submittedGraph?.['2'].inputs?.text).to.equal('a woman standing in a sunlit garden');
      expect(submittedGraph?.['3'].inputs?.text).to.equal('');
    } finally {
      socket.close();
      await new Promise<void>((resolve) => wsStub.wss.close(() => resolve()));
